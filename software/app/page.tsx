...
export default function Home() {
  ...
  return (
    <div className="shell">
      ...
      {/* Drawer backdrop */}
      <div className={`drawer-backdrop${menuOpen ? ' open' : ''}`} onClick={toggleMenu}></div>

      {/* Drawer */}
      <div className={`drawer${menuOpen ? ' open' : ''}`}>
        <div className="drawer-item" onClick={() => setMenuOpen(false)}>
          <div className="di-icon">
            <svg viewBox="0 0 24 24" fill="none"><circle cx="12" cy="8" r="4"/><path d="M4 20c0-4 3.6-7 8-7s8 3 8 7"/></svg>
          </div>My Account
        </div>
        <div className="drawer-item" onClick={() => setMenuOpen(false)}>
          <div className="di-icon">
        ...
        <div className="drawer-item" onClick={() => setMenuOpen(false)}>
          <div className="di-icon">
            <svg viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="9"/><line x1="12" y1="8" x2="12" y2="8.5" strokeWidth="2.2"/><line x1="12" y1="11" x2="12" y2="16"/></svg>
          </div>About H.A.L.O.
        </div>
        <div className="drawer-item danger" onClick={() => setMenuOpen(false)}>
          <div className="di-icon danger-icon">
      ...
      <div className="inner" id="mainInner">
        ...
        <div className={activePage === 'site' ? '' : 'page-hidden'}>
          <SitePage onOpenMap={() => setMapOpen(true)} />
        </div>
        <div className={activePage === 'modules' ? '' : 'page-hidden'}>
          <ModulesPage onToast={showToast} />
        </div>
        ...
        <button className={`nav-item${activePage === 'shifts' ? ' active' : ''}`} onClick={() => navTo('shifts')}>
          <div className="nav-icon">
            <svg className="nav-svg" viewBox="0 0 24 24">
              ...
              <rect x="7" y="14" width="3" height="3" rx=".5" style={{ fill: 'var(--muted)', stroke: 'none' }}/>
              <rect x="14" y="14" width="3" height="3" rx=".5" style={{ fill: 'var(--muted)', stroke: 'none' }}/>
            </svg>
          </div>
        ...
        <button className={`nav-item${activePage === 'site' ? ' active' : ''}`} onClick={() => navTo('site')}>
          <div className="nav-icon">
            <svg className="nav-svg" viewBox="0 0 24 24" fill="none">
              <path d="M2 6 L9 4 L9 19 L2 21 Z" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
              <path d="M9 4 L15 6 L15 19 L9 19 Z" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
              <path d="M15 6 L22 4 L22 19 L15 19 Z" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
              <line x1="3.5" y1="10" x2="7.5" y2="9.3" strokeWidth="1" opacity="0.6"/>
              <line x1="3.5" y1="13" x2="7.5" y2="12.3" strokeWidth="1" opacity="0.6"/>
              <line x1="16.5" y1="10" x2="20.5" y2="9.3" strokeWidth="1" opacity="0.6"/>
              <line x1="16.5" y1="14" x2="20.5" y2="13.3" strokeWidth="1" opacity="0.6"/>
            </svg>
          </div>
      ...
      <NotifScreen
        open={notifScreenOpen}
        onClose={() => setNotifScreenOpen(false)}
        onOpenAlert={() => { setNotifScreenOpen(false); setNotifOpen(true); }}
      />
